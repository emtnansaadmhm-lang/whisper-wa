...
# Blueprints
# ملاحظة: عطّلنا reports.py لأن عندكم نظام تقارير رسمي داخل database.py + routes هنا
try:
...
def get_pending_requests():
    ...
    return jsonify({"ok": True, "requests": requests_data}), 200
    ...
    if method == "wifi":
        if not ip_port:
            return jsonify({
                ...
                "step": "validate",
                "logs": logs,
                "error": "ip_port is required"
            }), 400

        logs.append({"ts": ts, "level": "INFO", "msg": f"ADB connect to {ip_port}..."})
        conn_result = adb_connect_wifi(ip_port, adb_path=adb_path)
...
def api_workflow_run():
    """
    يشغل: acquisition → decrypt → parse → analyze → build index
    POST body:
    {
      "case_id": "Case_001",
    ...
    if not acq.get("ok"):
        logs.append({"ts": ts, "level": "ERROR", "msg": "Acquisition failed."})
        return jsonify({
            "ok": False,
            "step": "acquisition",
            "logs": logs,
    ...
    if decrypt_whatsapp_db is not None:
        ...
        logs.append({"ts": ts, "level": "WARNING", "msg": "decrypt.py not available, skipping decryption."})
    ...
    messages_dict = {
        msg["id"]: str(msg.get("text", "") or "")
        for msg in parsed["messages"]
    }

    MESSAGES = messages_dict
    INDEX = build_index(messages_dict)

    logs.append({"ts": ts, "level": "SUCCESS", "msg": f"Indexed {len(INDEX.get('word_index', {}))} words."})
    result["indexed"] = {
        "total_words": len(INDEX.get("word_index", {})),
...
def search():
    ...
    if not INDEX:
        return jsonify({"ok": False, "error": "Search index is empty. Run workflow or build index first."}), 400
